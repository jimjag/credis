     ...
     static int cr_appendstrf(cr_buffer * buf, const char *format, ...)
{
    ...
    while (rc >= avail) {
        ...
        va_start(ap, format);
        rc = vsnprintf(buf->data + buf->len, buf->size - buf->len, format,
                       ap);
        va_end(ap);
 ...
 *   0  on success
 *  <0  on error, i.e. more memory not available */
static int cr_appendstrarray(cr_buffer * buf, int strc, const char **strv,
                             int newline)
{
    int rc, i;
...
static int cr_senddata(int fd, unsigned int msecs, char *buf, int size)
{
    ...
    /*
     * NOTE: On Linux, select() modifies timeout to reflect the amount of
     * time not slept, on other systems it is likely not the same
     */
...
static int cr_readln(REDIS rhnd, int start, char **line, int *idx)
{
    ...
    while (more > 0 ||
           (nl =
            cr_findnl(buf->data + buf->idx + start,
        ...
        rc = cr_receivedata(rhnd->fd, rhnd->timeout, buf->data + buf->len,
                            avail);
        if (rc > 0) {
...
static int cr_receivemultibulk(REDIS rhnd, char *line)
{
    ...
    if (bnum == -1) {
        ...
        if (cr_morebulk
            (&(rhnd->reply.multibulk), bnum - rhnd->reply.multibulk.size))
            return CREDIS_ERR_NOMEM;
    }

    for (i = 0; bnum > 0 && (rc = cr_readln(rhnd, 0, &line, &idx)) > 0;
         i++, bnum--) {
        type = *(line++);
        if (type == CR_BULK) {
            blen = atoi(line);
    ...
    for (i = 0; i < rhnd->reply.multibulk.len; i++) {
        if (rhnd->reply.multibulk.idxs[i] > 0)
            rhnd->reply.multibulk.bulks[i] =
                rhnd->buf.data + rhnd->reply.multibulk.idxs[i];
        else
...
REDIS cr_new(void)
{
    ...
    if ((rhnd = calloc(sizeof(cr_redis), 1)) == NULL ||
        ...
        (rhnd->reply.multibulk.bulks =
         malloc(sizeof(char *) * CR_MULTIBULK_SIZE)) == NULL
        || (rhnd->reply.multibulk.idxs =
            malloc(sizeof(int) * CR_MULTIBULK_SIZE)) == NULL) {
        cr_delete(rhnd);
        return NULL;
    }
...
__attribute__ ((format(printf, 3, 4)))
     static int cr_sendfandreceive(REDIS rhnd, char recvtype,
                                   const char *format, ...)
{
    int rc;
    va_list ap;
...
#ifdef WIN32
        /*
         * handle the fact that vnsprintf() returns -1 if the buffer is too
         * small
         */
        rc = buf->size * 2;
#else
...
static int cr_getredisversion(REDIS rhnd)
{
    if (cr_sendfandreceive(rhnd, CR_BULK, "INFO\r\n") == 0) {
        ...
        rhnd->version.number =
            CR_VERSION(rhnd->version.major, rhnd->version.minor,
                       rhnd->version.patch);
    }
...
REDIS credis_connect(const char *host, int port, int timeout)
{
    ...
    if (inet_aton(host, &sa.sin_addr) == 0) {
        ...
        memcpy(&sa.sin_addr.s_addr, &(info->ai_addr->sa_data[2]),
               sizeof(in_addr_t));
        freeaddrinfo(info);
    }
#endif
...
int credis_getset(REDIS rhnd, const char *key, const char *set_val,
                  char **get_val)
{
    int rc;
...
static int cr_multikeystorecommand(REDIS rhnd, const char *cmd,
                                   const char *destkey, int keyc,
                                   const char **keyv)
{
    cr_buffer *buf = &(rhnd->buf);
    int rc;
...
static int cr_incr(REDIS rhnd, int incr, int decr, const char *key,
                   int *new_val)
{
    int rc = 0;
...
int credis_substr(REDIS rhnd, const char *key, int start, int end,
                  char **substr)
{
    int rc;
...
int credis_getrange(REDIS rhnd, const char *key, int start, int end,
                    char **substr)
{
...
int credis_keys(REDIS rhnd, const char *pattern, char ***keyv)
{
    ...
    if (rhnd->version.number >= CR_VERSION(2, 0, 0)) {
        ...
        if ((rc =
             cr_sendfandreceive(rhnd, CR_BULK, "KEYS %s\r\n",
                                pattern)) == 0) {
            /*
...
{
    int rc = cr_sendfandreceive(rhnd, CR_INT, "PERSIST %s\r\n", key);
...
int credis_lrange(REDIS rhnd, const char *key, int start, int end,
                  char ***valv)
{
    int rc;
...
{
    int rc =
        cr_sendfandreceive(rhnd, CR_BULK, "LINDEX %s %d\r\n", key, index);
...
int credis_sort(REDIS rhnd, const char *query, char ***elementv)
{
    ...
    if ((rc =
         cr_sendfandreceive(rhnd, CR_MULTIBULK, "SORT %s\r\n", query)) == 0) {
...
int credis_info(REDIS rhnd, REDIS_INFO * info)
{
    ...
    if (rc == 0) {
        ...
        cr_parseinfo(rhnd->reply.bulk, "process_id", "%ld",
                     &(info->process_id));
        cr_parseinfo(rhnd->reply.bulk, "uptime_in_seconds", "%ld",
                     &(info->uptime_in_seconds));
        cr_parseinfo(rhnd->reply.bulk, "uptime_in_days", "%ld",
                     &(info->uptime_in_days));
        cr_parseinfo(rhnd->reply.bulk, "connected_clients", "%d",
                     &(info->connected_clients));
        cr_parseinfo(rhnd->reply.bulk, "connected_slaves", "%d",
                     &(info->connected_slaves));
        cr_parseinfo(rhnd->reply.bulk, "blocked_clients", "%d",
                     &(info->blocked_clients));
        cr_parseinfo(rhnd->reply.bulk, "used_memory", "%zu",
                     &(info->used_memory));
        cr_parseinfo(rhnd->reply.bulk, "used_memory_human",
                     "%" CR_USED_MEMORY_HUMAN_SIZE_STR "s",
                     &(info->used_memory_human));
        cr_parseinfo(rhnd->reply.bulk, "changes_since_last_save", "%lld",
                     &(info->changes_since_last_save));
        cr_parseinfo(rhnd->reply.bulk, "bgsave_in_progress", "%d",
                     &(info->bgsave_in_progress));
        cr_parseinfo(rhnd->reply.bulk, "last_save_time", "%ld",
                     &(info->last_save_time));
        cr_parseinfo(rhnd->reply.bulk, "bgrewriteaof_in_progress", "%d",
                     &(info->bgrewriteaof_in_progress));
        cr_parseinfo(rhnd->reply.bulk, "total_connections_received", "%lld",
                     &(info->total_connections_received));
        cr_parseinfo(rhnd->reply.bulk, "total_commands_processed", "%lld",
                     &(info->total_commands_processed));
        cr_parseinfo(rhnd->reply.bulk, "expired_keys", "%lld",
                     &(info->expired_keys));
        cr_parseinfo(rhnd->reply.bulk, "hash_max_zipmap_entries", "%zu",
                     &(info->hash_max_zipmap_entries));
        cr_parseinfo(rhnd->reply.bulk, "hash_max_zipmap_value", "%zu",
                     &(info->hash_max_zipmap_value));
        cr_parseinfo(rhnd->reply.bulk, "pubsub_channels", "%ld",
                     &(info->pubsub_channels));
        cr_parseinfo(rhnd->reply.bulk, "pubsub_patterns", "%u",
                     &(info->pubsub_patterns));
        cr_parseinfo(rhnd->reply.bulk, "keyspace_hits", "%lld",
                     &(info->keyspace_hits));
        cr_parseinfo(rhnd->reply.bulk, "keyspace_misses", "%lld",
                     &(info->keyspace_misses));
        cr_parseinfo(rhnd->reply.bulk, "vm_enabled", "%d",
                     &(info->vm_enabled));
        cr_parseinfo(rhnd->reply.bulk, "role", "%c", &role);

        info->role =
            ((role == 'm') ? CREDIS_SERVER_MASTER : CREDIS_SERVER_SLAVE);
    }
...
static int cr_setaddrem(REDIS rhnd, const char *cmd, const char *key,
                        const char *member)
{
    int rc;
...
int credis_sinterstore(REDIS rhnd, const char *destkey, int keyc,
                       const char **keyv)
{
    return cr_multikeystorecommand(rhnd, "SINTERSTORE", destkey, keyc, keyv);
}

int credis_sunionstore(REDIS rhnd, const char *destkey, int keyc,
                       const char **keyv)
{
    return cr_multikeystorecommand(rhnd, "SUNIONSTORE", destkey, keyc, keyv);
}

int credis_sdiffstore(REDIS rhnd, const char *destkey, int keyc,
                      const char **keyv)
{
...
int credis_zincrby(REDIS rhnd, const char *key, double incr_score,
                   const char *member, double *new_score)
{
    int rc;
...
static int cr_zrank(REDIS rhnd, int reverse, const char *key,
                    const char *member)
{
    int rc;
...
int cr_zrange(REDIS rhnd, int reverse, const char *key, int start, int end,
              char ***elementv)
{
    int rc = cr_sendfandreceive(rhnd, CR_MULTIBULK, "%s %s %d %d\r\n",
                                reverse == 1 ? "ZREVRANGE" : "ZRANGE", key,
                                start, end);
...
int credis_zrange(REDIS rhnd, const char *key, int start, int end,
                  char ***elementv)
{
    return cr_zrange(rhnd, 0, key, start, end, elementv);
}

int credis_zrevrange(REDIS rhnd, const char *key, int start, int end,
                     char ***elementv)
{
    return cr_zrange(rhnd, 1, key, start, end, elementv);
}

int cr_zrangebyscore(REDIS rhnd, int reverse, const char *key, double a,
                     double b, char ***elementv)
{
    int rc = cr_sendfandreceive(rhnd, CR_MULTIBULK, "%s %s %f %f\r\n",
                                reverse ==
                                1 ? "ZREVRANGEBYSCORE" : "ZRANGEBYSCORE", key,
                                a, b);
...
int credis_zrangebyscore(REDIS rhnd, const char *key, double min, double max,
                         char ***elementv)
{
    return cr_zrangebyscore(rhnd, 0, key, min, max, elementv);
}

int credis_zrevrangebyscore(REDIS rhnd, const char *key, double max,
                            double min, char ***elementv)
{
...
int credis_zscore(REDIS rhnd, const char *key, const char *member,
                  double *score)
{
    int rc;
...
int credis_zremrangebyscore(REDIS rhnd, const char *key, double min,
                            double max)
{
...
static int cr_zstore(REDIS rhnd, int inter, const char *destkey, int keyc,
                     const char **keyv, const int *weightv,
                     REDIS_AGGREGATE aggregate)
{
    cr_buffer *buf = &(rhnd->buf);
    int rc, i;
    ...
    if ((rc =
         cr_appendstrf(buf, "%s %s %d", inter ? "ZINTERSTORE" : "ZUNIONSTORE",
                       destkey, keyc)) != 0)
        return rc;
...
int credis_zinterstore(REDIS rhnd, const char *destkey, int keyc,
                       const char **keyv, const int *weightv,
                       REDIS_AGGREGATE aggregate)
{
...
int credis_zunionstore(REDIS rhnd, const char *destkey, int keyc,
                       const char **keyv, const int *weightv,
                       REDIS_AGGREGATE aggregate)
{
...
int credis_hset(REDIS rhnd, const char *key, const char *field,
                const char *value)
{
    int rc;
...
int credis_hmget(REDIS rhnd, const char *key, int fieldc, const char **fieldv,
                 char ***valv)
{
    cr_buffer *buf = &(rhnd->buf);
    int rc, i;
...
int credis_hmset(REDIS rhnd, const char *key, int fieldc, const char **fieldv,
                 const char **valv)
{
    cr_buffer *buf = &(rhnd->buf);
    int rc, i;
    ...
    for (i = 0; i < fieldc; i++) {
        if ((rc =
             cr_appendstrf(buf, "$%zu\r\n%s\r\n$%zu\r\n%s\r\n",
                           strlen(fieldv[i]), fieldv[i], strlen(valv[i]),
                           valv[i])) != 0)
            return rc;
    }
...
int credis_hincrby(REDIS rhnd, const char *key, const char *field,
                   int incr_val, int *new_val)
{
    int rc = 0;
    rc = cr_sendfandreceive(rhnd, CR_INT, "HINCRBY %s %s %d\r\n",
                            key, field, incr_val);
...
{
    int rc = cr_sendfandreceive(rhnd, CR_MULTIBULK, "HVALS %s\r\n", key);
 ...
 * is returned. */
static cr_message *cr_storemessage(REDIS rhnd, char *pattern, char *channel,
                                   char *message)
{
    cr_message *msg;
...
static int cr_parsepubsubmessage(REDIS rhnd, char **pattern, char **channel,
                                 char **message)
{
...
static int cr_sendandwaitforpubsub(REDIS rhnd, const char *command,
                                   const char *data)
{
    cr_buffer *buf = &(rhnd->buf);
    ...
    while (rc == 0) {
        ...
        else if (cr_parsepubsubmessage(rhnd, &pattern, &channel, &message) ==
                 0) {
            if (cr_storemessage(rhnd, pattern, channel, message) == NULL)
                return CREDIS_ERR_NOMEM;
        }
        else;                   /* TODO out of order pub/sub message, return
                                 * error or silently ignore? */
    }
